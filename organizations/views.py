...
def org_unit_list_view(request):
    ...
    if user.role == "SUPER_ADMIN":
        return render(
            request,
            "organizations/org_unit_list.html",
            {
                "is_super_admin": True,
    ...
    try:
        with schema_context(user.ministry_schema):
            ...
            all_units = list(OrgUnit.objects.all().order_by("unit_type", "name"))
...
def org_unit_create_view(request):
    ...
    if request.method == "POST":
        ...
        if errors:
            ...
            return render(
                request,
                "organizations/org_unit_form.html",
                {
        ...
        try:
            ...
            messages.success(
                request, f"{unit_type.title()} '{name}' created successfully."
            )
...
def org_unit_edit_view(request, unit_id):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            return render(
                request,
                "organizations/org_unit_edit.html",
                {
                    "org_unit": org_unit,
                    "parent_units": parent_units,
...
def audit_log_detail_view(request, log_id):
    """
    Show full details of a single audit log entry including
    before/after value comparison.
    
    Super Admin cannot view ministry-level log details
    (they have no ministry schema to query).
    """
    user = request.user
    log = None
    ...
    if log.old_value or log.new_value:
        ...
        # Collect all field names that appear in either old or new
        ...
        for field in all_fields:
            ...
            changes.append({
                ...
                'old_val': old_val if old_val not in (None, '') else '—',
                'new_val': new_val if new_val not in (None, '') else '—',
                'changed': str(old_val) != str(new_val),
            })

    return render(request, 'organizations/audit_log_detail.html', {
        'log':        log,
        'changes':    changes,
        'page_title': f'Audit Log #{log.id}',
    })
