"""Web views for master data and asset category management."""
...
def master_data_list_view(request):
    ...
    return render(request, 'organizations/master_data_list.html', {
...
def master_data_create_view(request):
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            return render(request, 'organizations/master_data_form.html', {
                'categories': categories,
        ...
        try:
            with schema_context(user.ministry_schema):
                ...
                if MasterData.objects.filter(
                    ...
                    messages.error(
                        request,
                        f"Value '{value}' already exists in this category."
                    )
                    return render(request, 'organizations/master_data_form.html', {
                        'categories': categories,
                        'page_title': 'Add Master Data',
            ...
            messages.success(
                request,
                f"'{label}' added to {category} successfully."
            )
...
def master_data_edit_view(request, item_id):
    ...
    if request.method == 'POST':
        ...
        if not label:
            ...
            return render(request, 'organizations/master_data_edit.html', {
                'item':       item,
                'categories': categories,
...
def asset_category_list_view(request):
    """List all asset categories for this ministry."""
    user = request.user
    categories = []
    ...
    try:
        with schema_context(user.ministry_schema):
            ...
            categories = list(
                AssetCategory.objects.all().order_by('name')
            )
    except Exception as e:
        messages.error(request, f"Error loading categories: {str(e)}")

    return render(request, 'organizations/asset_category_list.html', {
        'categories': categories,
...
def asset_category_create_view(request):
    """Create a new asset category. The code is used in auto-generated asset numbers (e.g., ICT → MOH-ICT-2024-0001)."""
    user = request.user
    ...
    if request.method == 'POST':
        ...
        if errors:
            for error in errors:
                messages.error(request, error)
            return render(request, 'organizations/asset_category_form.html', {
        ...
        try:
            with schema_context(user.ministry_schema):
                ...
                if AssetCategory.objects.filter(code=code).exists():
                    messages.error(
                        request,
                        f"Code '{code}' is already used by another category."
                    )
                    return render(request, 'organizations/asset_category_form.html', {
                        'page_title': 'Add Asset Category',
                        'form_data':  request.POST,
...
def asset_category_edit_view(request, category_id):
    """Edit an asset category. Code cannot be changed after creation (it's embedded in existing asset numbers)."""
    user = request.user
    category = None
    ...
    if request.method == 'POST':
        ...
        if not name:
            ...
            return render(request, 'organizations/asset_category_edit.html', {
                'category':   category,
